...
func (g *GuiOutput) clear(targetDisplay *gtk.TextView) {
...
func (g *GuiOutput) display(purpose Purpose, targetDisplay *gtk.TextView, msg string, tag *gtk.TextTag) {
	var line string
	var beep bool
	switch purpose {
	case INFO:
	...
	case CONTACT:
		...
		buffer.InsertWithTag(&iter, line, tag)
		gdk.ThreadsLeave()
		return
	case STATUS:
		line = msg + "\n\n"
...
func (s *Session) guiProcessPresence(stanza *xmpp.ClientPresence) {
	...
	case "subscribe":
		...
		g.display(INFO, s.statusTabView, jid+" wishes to see when you're online. Use '/confirm "+jid+"' to confirm (or likewise with /deny to decline)", nil)
	...
	g.display(STATUS, s.statusTabView, fmt.Sprintf("%v", string(prefix)+string(guiLine)), nil)
	...
	for _, item := range s.roster {
		...
		if (ok && (state == "")) || (state == "chat") {
			g.display(CONTACT, s.contactsView, line, tagGreen)
		} else if ok && (state != "") {
			g.display(CONTACT, s.contactsView, line, tagYellow)
		} else {
			g.display(CONTACT, s.contactsView, line, tagRed)
		}
	}
}
